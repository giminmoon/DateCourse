...
class SignUpViewController: UIViewController, UITextFieldDelegate {
    ...
    static func newSignUpViewController() -> SignUpViewController {
        ...
        if (!isFieldEmpty()) {
            Auth.auth().createUser(withEmail: usernameField.text!, password: passwordField.text!, completion: {(user,error) in
                ...
                if error != nil {
                    ...
                    signuperrorAlert.addAction(UIAlertAction(title: "OK", style: .cancel, handler: nil))
            ...
            let passNotMatch = UIAlertController(title: "Error!", message: "Please Fill Out All Fields!", preferredStyle: .alert)
    ...
    func isFieldEmpty() -> Bool {
        if ((usernameField.text?.isEmpty)! || (passwordField.text?.isEmpty)! || (firstnameField.text?.isEmpty)! || (lastNameField.text?.isEmpty)!)
        {
            return true
        }
        return false
